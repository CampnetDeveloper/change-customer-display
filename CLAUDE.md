...
- **リリースビルド作成時は必ずバージョンを上げる**
  - `pubspec.yaml` の `version: X.Y.Z+N` を更新
  - 例: `1.0.0+1` → `1.0.1+2`（バグ修正）、`1.1.0+3`（機能追加）
- バージョン形式: `メジャー.マイナー.パッチ+ビルド番号`
...
| 優先度 | アクション | 内容 | 状態 |
|--------|-----------|------|------|
| **1** | 実機テスト | Android端末にAPKをインストールして動作確認 | ⏳ 未着手 |
| **2** | iPad接続テスト | 実際のiPad POSからWebSocket接続テスト | ⏳ 未着手 |
| **3** | Google Play公開 | 署名キー作成 → AABビルド → 審査申請 | ⏳ 未着手 |
| **4** | 機能追加 | 要望があれば対応 | ⏳ 未着手 |
